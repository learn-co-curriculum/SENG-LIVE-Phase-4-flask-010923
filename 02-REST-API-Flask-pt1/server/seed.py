...
with app.app_context():   
    ...
    cats_cast_members = [CastMembers(name=fake.name(), role=role, production_id=p2.id) for role in cats_roles]
    db.session.add_all(cats_cast_members)
    db.session.commit()

    carmen_roles = ['Carmen', 'Escamillo', 'Jose', 'Mercedes', 'Dancaire']
    carmen_cast_members = [CastMembers(name=fake.name(), role=role, production_id=p3.id) for role in carmen_roles]
    ...
    hamilton_cast_members = [CastMembers(name=fake.name(), role=role, production_id=p4.id) for role in hamilton_roles]
